### 20170308-신상훈
>교차점을 추출해서 교차점에 빨간 점 & 초록 점을 찍는다(점이 매우 작은 pixel이라 확대해서 확인 해야함.)

### 20170224 박성진
> 윤곽선을 추출해서 그려보고 윤곽선을 세밀하게 다듬어서 해당 물체가 무엇인지(ex:책)를 판단할 수 있는 엔진을 실행함 그리고 윤곽선 근사치를 적용하여 해당 윤곽선이 꼭지점 4개인 직사각형인지를 판단후 책을 찾아내어 개수 파악후 정확히 몇개인지까지도 확인함
